...
function g_rect(inputFname,image_num,nostop)
...
if exist('ship','var')
   [xp,yp] = g_ll2pix(ship.lon,ship.lat,imgWidth,imgHeight,...
                     lambda,phi,theta,H,LON0,LAT0,frameRef,lens);
 
   hS=line(xp,yp,'color',[0 .5 0],'marker','.');
   
   [xp,yp] = g_ll2pix(ship.Clon,ship.Clat,imgWidth,imgHeight,...
                     lambda,phi,theta,H,LON0,LAT0,frameRef,lens);
 
   hSC=line(xp,yp,'color',[0 .5 0],'marker','s','markersize',10);
end   
...
save([outputDir outputFname],'imgDir','imgFname','firstImgFname',...
                 'lastImgFname','frameRef',...
                 'LON','LAT',...
                 'LON0','LAT0',...
                 'lon_gcp0','lat_gcp0',...
...
if exist('ship','var')
    m_line(ship.lon,ship.lat,'color','r');
    m_line(ship.Clon,ship.Clat,'color','r','marker','o');
end
