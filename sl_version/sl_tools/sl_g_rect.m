...
%  Thunderbird Sports Field, 24 labels per image
 ...
 %    lens.jc=0;      % x direction   
    ...
    % SL - July 23, 2026: Intercept the native axes toolbar 'Home' button click
    try
        % Access the axes toolbar (MATLAB R2018b and newer)
        tb = axtoolbar(gca, {'pan', 'zoomin', 'zoomout', 'restoreview'});

        % Find the 'restoreview' (Home) button and attach your reset callback
        btnHome = findobj(tb, 'Tag', 'MATLAB.UI.AXESTOOLBAR.RESTOREVIEW');
        if ~isempty(btnHome)
            set(btnHome, 'ButtonPushedFcn', @resetZoomView);
        end
    catch
        % Fallback for older MATLAB versions: attach listener to standard figure mode
        hZoom = zoom(gcf);
        set(hZoom, 'ActionPreCallback', @handlePreZoomAction);
    end
    ...
    % SL - July 23, 2026: Reset manual custom zoom when changing images, 
    % UNLESS Horizon Zoom is explicitly toggled ON.
    if persistentHorizonZoomOn
        % Horizon Zoom takes priority and stays active for the new image frame
        applyCustomZoomLimits([], []);
    else
        % Clear manual/custom text boxes and start the new image in full view
        resetZoomView([], []);
    end
    ...
    if strcmp(nostop,'gui')
        ...
        yawmode  =uicontrol(gcf,'style','slider','tag','yaw',  'unit','normalized','position',[.1 sliderY  sliderWidth sliderHeight],...
            'value',0,'max',180,'min',-180,'sliderstep',[.01 .1]);
...
% SL - July 23, 2026: Safely clears zoom parameters and handles pre-UI execution
    function resetZoomView(~, ~)
        % 1. Only attempt to clear text fields if they have already been constructed in the UI
        if exist('hXMin', 'var') && ishandle(hXMin)
            set(hXMin, 'String', '');
            set(hXMax, 'String', '');
            set(hYMin, 'String', '');
            set(hYMax, 'String', '');
        ...
        % 3. Safely disengage Horizon Zoom toggle if it was running
        persistentHorizonZoomOn = false;
        if exist('gcf', 'builtin')
            hHzToggle = findobj(gcf, 'Tag', 'hzToggle');
            if ~isempty(hHzToggle) && ishandle(hHzToggle)
                set(hHzToggle, 'Value', 0, 'String', 'Horizon Zoom: OFF', 'BackgroundColor', [0.94 0.94 0.94]);
            end
        end
        
        % 4. Force image axes back to full dimensions [1, imgWidth] & [1, imgHeight]
        if exist('imgWidth', 'var') && exist('imgHeight', 'var')
            xlim(gca, [1, imgWidth]);
            ylim(gca, [1, imgHeight]);
        end
    end

% SL - July 23, 2026: Helper function to safely catch pre-zoom actions on older MATLAB versions
    function handlePreZoomAction(~, evt)
        if isprop(evt, 'Axes') && strcmp(evt.Axes.Tag, 'home')
            resetZoomView([], []);
        end
    end
end
